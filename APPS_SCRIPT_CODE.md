# Apps Script Code for OpenGrad Scheduling
...
6. **IMPORTANT**: Update line 13 - Replace `YOUR_SHEET_ID_HERE` with your actual Google Sheet ID
7. Click **Save** (💾 icon)
8. Deploy as Web App (see SETUP_GUIDE.md for details)
...
1. **Update Sheet ID**: Before using, replace `YOUR_SHEET_ID_HERE` on line 13 with your actual Google Sheet ID
   - Find it in your Google Sheet URL: `https://docs.google.com/spreadsheets/d/SHEET_ID_HERE/edit`

2. **Sheet Names**: Make sure your Google Sheet has these exact sheet names (case-sensitive):
   - `Mentors`
   - `Students`
   - `Slots`

3. **Permissions**: When you first run the script, Google will ask for permissions:
   - Google Sheets (read/write)
   - Google Calendar (create events)
   - Gmail (send emails)

4. **Deploy as Web App**: After pasting the code:
   - Click **Deploy > New deployment**
   - Select **Web app**
   - Set "Who has access" to "Anyone" (or "Anyone with Google account")
   - Copy the Web App URL and add it to your `.env.local` as `NEXT_PUBLIC_APPS_SCRIPT_URL`
